<template>
  <div :style="!$route.meta.hiddenHeaderContent ? 'margin: -24px -24px 0px;' : null">
    <PageHeader v-if="!$route.meta.hiddenHeaderContent"
                 :title="pageTitle"
                 :logo="logo"
                 :avatar="avatar">
      <slot slot="action"
      ...
      <slot slot="extra"
            name="extra">
        <div class="extra-img">
          <img v-if="typeof extraImage !== 'undefined'"
               :src="extraImage" />
        </div>
      </slot>
      <div slot="pageMenu">
        <div class="page-menu-search"
             v-if="search">
          <a-input-search style="width: 80%; max-width: 522px"
                          placeholder="请输入..."
                          size="large"
                          enterButton="搜索" />
        </div>
